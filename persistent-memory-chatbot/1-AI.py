...
def update_memory():
    identity = subprocess.Popen(["python3", os.path.join(BASE_DIR, "Identity-AI.py")])
    knowledge = subprocess.Popen(["python3", os.path.join(BASE_DIR, "Knowledge-AI.py")])

    identity.wait()
    knowledge.wait()

    personal = subprocess.Popen(["python3", os.path.join(BASE_DIR, "Personal-AI.py")])
    context = subprocess.Popen(["python3", os.path.join(BASE_DIR, "Context-AI.py")])

    personal.wait()
    context.wait()

    subprocess.run(["python3", os.path.join(BASE_DIR, "categorie.py")])
